...
app.get('/', (req, res) => {
    res.json("Welcome to the Edmund Scraper API, list -> model -> load")
})
...
Model_webList_local.forEach(item => {
    axios.get(item.url)
        .then(response => {
            ...
            const url_image = $("#main-content > div.pb-md-3_25 > div:nth-child(1) > div > div.mt-md-1.pr-xl-2.col-12.col-md-7.offset-md-0.col-lg-8 > section > div:nth-child(2) > div > div > div > div > div > div.slick-slide.slick-active.slick-current > div > div > div > div > img").attr('src')
